...
@Composable
fun ProfileSetScreenBrightness(
    brightness: Int,
    onDismiss: () -> Unit,
    onAccept: (Int) -> Unit,
    onCancel: () -> Unit
    ...
    DialogWrapper(onDismiss = onDismiss) {
        ...
        DialogCheckedPreference(
            desc = stringResource(id = R.string.dialog_profile_set_screen_brightness_auto),
            checked = auto.value,
        ...
        Slider(
            value = if (auto.value) 0f else intensity.value.toFloat(),
            enabled = !auto.value,
            valueRange = 0f..100f,
        ...
        Text(
            if (!auto.value) "${intensity.value}%" else stringResource(id = R.string.profile_screen_brightness_auto),
